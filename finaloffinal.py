...
def listen():
    ...
    with sr.Microphone() as source:
        status_label.configure(text="Press and hold space to speak...")
        speak("Press and hold space to speak")
        ...
        root.after(100, listen)  # Continue listening
...
def handle_voice_input(event=None):
    global order_items, total_order_price
    speak("Press and hold space to speak, release to stop. Say 'generate bill' when done.")
    order_items = []
    total_order_price = 0
    root.after(100, listen)  # Start listening after a short delay
...
speak("This System is now Active")
...
#For space operated mic
space_pressed = ctk.BooleanVar()
space_pressed.set(False)
...
root.bind('<j>', handle_voice_input) 

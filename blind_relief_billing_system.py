...
def record_sale(item_name, quantity, total_price):
    cursor.execute("UPDATE items SET stock = stock - %s WHERE item_name = %s", (quantity, item_name))
    ...
    cursor.execute("SELECT ingredient_name, quantity FROM ingredients WHERE item_id = (SELECT item_id FROM items WHERE item_name = %s)", (item_name,))
    ingredients = cursor.fetchall()
    for ingredient, ing_quantity in ingredients:
        cursor.execute("UPDATE items SET stock = stock - %s WHERE item_name = %s", (ing_quantity * quantity, ingredient))
...
def add_stock():
    item_name = simpledialog.askstring("Add Stock", "Enter the item name:")
    if item_name:
        quantity = simpledialog.askinteger("Add Stock", "Enter the quantity to add:")
        if quantity:
            try:
                cursor.execute("UPDATE items SET stock = stock + %s WHERE item_name = %s", (quantity, item_name))
                db.commit()
                speak(f"Added {quantity} to the stock of {item_name}.")
            except mysql.connector.Error as error:
                speak(f"Error updating stock: {error}")

def get_item_stock(item_name):
    cursor.execute("SELECT stock FROM items WHERE item_name = %s", (item_name,))
    result = cursor.fetchone()
    return result[0] if result else None


def display_stock():
    cursor.execute("SELECT item_name, stock FROM items")
    stock_data = cursor.fetchall()
    if stock_data:
        df = pd.DataFrame(stock_data, columns=['Item', 'Stock'])
        file_name = f'stock_report_{datetime.today().date().strftime("%d-%m-%Y")}.xlsx'
        df.to_excel(file_name, index=False, header=True)
        speak("Stock report has been generated.")
        os.startfile(file_name)
    else:
        speak("No stock data available.")
        ...
        if command:
            if command.lower() == "generate bill":
                ...
                try:
                    ...
                    current_stock = get_item_stock(item_name)
                    if current_stock is None:
                        speak(f"Item {item_name} not found.")
                        continue
                    
                    if quantity > current_stock:
                        speak(f"Sorry, only {current_stock} {item_name}(s) available in stock.")
                        speak("Would you like to order the available quantity? Say 'yes' or 'no'.")
                        ...
                        while True:
                            ...
                            if response:
                                if response.lower() == "yes":
                                    ...
                                    speak("Order cancelled for this item.")
                                    break
...
add_stock_button = ctk.CTkButton(side_frame, text="Add Stock", command=add_stock, font=("Arial", 12), fg_color="white", text_color="black")
add_stock_button.pack(pady=10)

display_stock_button = ctk.CTkButton(side_frame, text="Display Stock", command=display_stock, font=("Arial", 12), fg_color="white", text_color="black")
display_stock_button.pack(pady=10)
